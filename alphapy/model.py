...
from alphapy.estimators import scorers, xgb_score_map
...
from alphapy.utilities import get_datestamp, most_recent_file
...
class Model:
    ...
    def __init__(self, specs):
        ...
        try:
            ...
            raise KeyError("Model specs must include the key: algorithms") from e
        self.best_algo = None
        # feature names
        self.feature_names = []
        # feature map
...
def get_model_config() -> dict[str, Any]:
    ...
    if sampling_method in samplers:
        ...
        raise ValueError(f"model.yml data:sampling:method {sampling_method} unrecognized")
    ...
    if encoder in encoders:
        ...
        raise ValueError(f"model.yml features:encoding:type {encoder} unrecognized")
    # factors
    ...
    if scaler_type in scaler_types:
        ...
        raise ValueError(f"model.yml features:scaling:type {scaler_type} unrecognized")
    # SciPy
    ...
    if model_type in model_types:
        ...
        raise ValueError(f"model.yml model:type {model_type} unrecognized")
    # end of model type
    ...
    if score_func in feature_scorers:
        ...
        raise ValueError(f"model.yml model:feature_selection:score_func {score_func} unrecognized")
    # grid search
...
def load_predictor(directory: str) -> Any:
    ...
    if file_ext == "pkl" or file_ext == "h5":
        ...
        if file_ext == "pkl":
            ...
            if KERAS_AVAILABLE:
                ...
                raise ImportError(
                    "Cannot load .h5 model file. TensorFlow/Keras is not installed. "
                    "Install with: uv sync --group tensorflow"
                )
    else:
        logging.error("Could not find model predictor in %s", search_dir)
...
def save_predictor(model: "Model", timestamp: str) -> None:
...
def load_feature_map(model: "Model", directory: str) -> "Model":
...
def save_feature_map(model: "Model", timestamp: str) -> None:
...
def first_fit(model: "Model", algo: str, est: Any) -> Any:
    ...
    if algo_xgb and scorer in xgb_score_map:
        X1, X2, y1, y2 = train_test_split(X_train, y_train, test_size=split, random_state=seed)
        eval_set = [(X1, y1), (X2, y2)]
        eval_metric = xgb_score_map[scorer]
        est.fit(X1, y1, eval_set=eval_set, eval_metric=eval_metric, early_stopping_rounds=esr)
    else:
    ...
    try:
        scores = cross_val_score(est, X_train, y_train, scoring=scorer, cv=cv_folds, n_jobs=n_jobs, verbose=verbosity)
        logger.info("Cross-Validation Scores: %s", scores)
    except (ValueError, TypeError, Exception) as e:
        logger.info("Cross-Validation Failed: %s. Try setting number_jobs = 1 in model.yml", e)
...
def make_predictions(model: "Model", algo: str, calibrate: bool) -> "Model":
...
def predict_best(model: "Model") -> "Model":
    ...
    maximize = scorers[scorer][1] == Objective.maximize
    best_score = -sys.float_info.max if maximize else sys.float_info.max
...
def predict_blend(model: "Model") -> "Model":
...
def generate_metrics(model: "Model", partition: Any) -> "Model":
    ...
    expected = model.y_train if partition == Partition.train else model.y_test
    ...
    if expected.any():
        ...
        for algo in algolist:
            ...
            # regression metrics
                ...
                try:
                    model.metrics[(algo, partition, "explained_variance")] = explained_variance_score(
                        expected, predicted
                    )
                except (ValueError, TypeError):
                    logger.info("Explained Variance Score not calculated")
                try:
                    model.metrics[(algo, partition, "neg_mean_absolute_error")] = mean_absolute_error(
                        expected, predicted
                    )
                except (ValueError, TypeError):
                    logger.info("Mean Absolute Error not calculated")
                try:
                    model.metrics[(algo, partition, "neg_median_absolute_error")] = median_absolute_error(
                        expected, predicted
                    )
                except (ValueError, TypeError):
                ...
                try:
                    model.metrics[(algo, partition, "neg_mean_squared_log_error")] = mean_squared_log_error(
                        expected, predicted
                    )
                except (ValueError, TypeError):
...
def save_predictions(model: "Model", tag: str, partition: Any) -> tuple[NDArray[Any], NDArray[Any] | None]:
...
def save_model(model: "Model", tag: str, partition: Any) -> None:
